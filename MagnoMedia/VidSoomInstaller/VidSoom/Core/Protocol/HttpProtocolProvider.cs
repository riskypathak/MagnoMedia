...
namespace MyDownloader.Protocols
{
    public class HttpProtocolProvider : BaseProtocolProvider, IProtocolProvider
    {
        ...
        public virtual RemoteFileInfo GetFileInfo(ResourceLocation rl, out Stream stream)
        {
            ...
            result.FileName = response.Headers["Content-Disposition"];
            if (result.FileName != null)
            {
                result.FileName = result.FileName.Substring(result.FileName.IndexOf("filename=\"") + 10);
                result.FileName = result.FileName.Substring(0, result.FileName.LastIndexOf("\""));
            }
            else
            {
                result.FileName = rl.URL.Substring(rl.URL.LastIndexOf('/') + 1);
            }
